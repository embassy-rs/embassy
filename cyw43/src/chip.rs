...
pub async fn device_core_is_up(bus: &mut impl Bus, chip: impl Chip, core: Core) -> Result<(), ()> {
...
pub async fn disable_device_core(bus: &mut impl Bus, chip: impl Chip, core: Core, halt: bool) -> Result<(), ()> {
    let base = chip.base_addr(core);
...
/// Resets the core identified by the provided coreId
pub async fn reset_device_core(bus: &mut impl Bus, chip: impl Chip, core: Core, halt: bool) -> Result<(), ()> {
    let base = chip.base_addr(core);
...
pub async fn chip_specific_socsram_init(bus: &mut impl Bus, chip: impl Chip) -> Result<(), ()> {
