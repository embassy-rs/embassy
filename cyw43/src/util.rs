...
pub(crate) const fn slice8_mut(x: &mut Aligned<A4, [u8]>) -> &mut [u8] {
    unsafe { slice::from_raw_parts_mut(x as *mut _ as *mut u8, size_of_val(x)) }
}

pub(crate) const fn slice8_ref(x: &Aligned<A4, [u8]>) -> &[u8] {
    unsafe { slice::from_raw_parts(x as *const _ as *const u8, size_of_val(x)) }
}

pub(crate) const fn aligned_mut<const N: usize>(x: &mut Aligned<A4, [u8; N]>) -> &mut Aligned<A4, [u8]> {
    unsafe { mem::transmute(slice::from_raw_parts_mut(x as *mut _ as *mut u8, N)) }
}

pub(crate) const fn aligned_ref<const N: usize>(x: &Aligned<A4, [u8; N]>) -> &Aligned<A4, [u8]> {
    unsafe { mem::transmute(slice::from_raw_parts(x as *const _ as *const u8, N)) }
}
