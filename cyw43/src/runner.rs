...
pub(crate) trait SealedBus {
    ...
    #[allow(unused)]
    async fn bp_read(&mut self, addr: u32, buf: &mut Aligned<A4, [u8]>);
    async fn bp_write(&mut self, addr: u32, data: &Aligned<A4, [u8]>);
    ...
    BUS: Bus,
{
    ...
    async fn log_init(&mut self) {
        ...
        let mut shared = Aligned([0; SharedMemData::SIZE]);
    ...
    async fn log_read(&mut self) {
        // Read log struct
        let mut log = Aligned([0; SharedMemLog::SIZE]);
