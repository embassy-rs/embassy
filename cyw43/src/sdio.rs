...
use crate::util::{aligned_mut, aligned_ref, slice32_ref, try_until};
...
/// Doc
pub struct SdioBus<SDIO, PIN> {
    backplane_window: u32,
    sdio: SDIO,
    pin: PIN,
}

impl<SDIO, PIN> SdioBus<SDIO, PIN>
where
    SDIO: SdioBusCyw43<BLOCK_SIZE>,
    PIN: Wait + InputPin,
{
    pub(crate) fn new(sdio: SDIO, pin: PIN) -> Self {
        Self {
    ...
    async fn cmd52(&mut self, write: bool, func: u32, addr: u32, val: u8) -> u8 {
        ...
        let result = self.sdio.cmd52(arg).await.unwrap_or(u16::MAX);
    ...
    async fn cmd53_write(&mut self, func: u32, addr: u32, buf: &Aligned<A4, [u8]>) {
        ...
        // trace!("cmd53 write: 0x{:08x} (len: {})", arg, buf.len());

        match mode {
            Mode::Byte => {
                if self.sdio.cmd53_byte_write(arg, buf).await.is_err() {
                    debug!("cmd53 byte write failed (len: {})", buf.len());
                }
            }
            Mode::Block => {
                if self
    ...
    async fn cmd53_read(&mut self, func: u32, addr: u32, buf: &mut Aligned<A4, [u8]>) {
        ...
        // trace!("cmd53 read: 0x{:08x} (len: {})", arg, buf.len());

        match mode {
            Mode::Byte => {
                if self.sdio.cmd53_byte_read(arg, buf).await.is_err() {
                    debug!("cmd53 byte read failed (len: {})", buf.len());
                }
            }
            Mode::Block => {
                if self
...
impl<SDIO, PIN> SealedBus for SdioBus<SDIO, PIN>
where
    ...
    PIN: Wait + InputPin,
{
    ...
    async fn wlan_read(&mut self, buf: &mut Aligned<A4, [u8]>) {
        ...
        if block_part > 0 {
            self.cmd53_read(FUNC_WLAN, address as u32, &mut buf[..block_part]).await;
    ...
    async fn wlan_write(&mut self, buf: &Aligned<A4, [u8]>) {
        ...
        if block_part > 0 {
            self.cmd53_write(FUNC_WLAN, address as u32, &buf[..block_part]).await;
    ...
    #[allow(unused)]
    async fn bp_read(&mut self, mut addr: u32, mut data: &mut Aligned<A4, [u8]>) {
        unimplemented!()
    }

    /// whd_bus_download_resource
    async fn bp_write(&mut self, mut addr: u32, data: &Aligned<A4, [u8]>) {
        ...
        // align data length to 4
        let mut data = aligned_ref(slice32_ref(data));
        ...
        while !data.is_empty() {
...

            {
                let byte_part = size_of_val(buf) % BLOCK_SIZE;
                let block_part = size_of_val(buf) - byte_part;
                let mut addr = addr as usize & BACKPLANE_ADDRESS_MASK as usize;

                if block_part > 0 {
                    self.cmd53_write(FUNC_BACKPLANE, addr as u32, &buf[..block_part]).await;
