...
impl<'d> Watchdog<'d> {
    ...
    pub fn start(&mut self, instruction_timer_value: u32, secure_counter_value: u32) {
        ...
        self.info.reload().write(|w| unsafe {  w.rload().bits(instruction_timer_value) });
        // Start the watchdog with initial secure counter value
        self.info.start().write(|w| unsafe {  w.strt().bits(secure_counter_value) });
    }
    ...
    pub fn check(&mut self, check: u32) {
        ...
        self.info.start().write(|w| unsafe {  w.strt().bits(self.secure_counter) });
    }
