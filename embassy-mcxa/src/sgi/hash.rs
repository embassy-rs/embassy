...
use super::sgi::{SGIError, Sgi};

// ============================================================================
// Public constants re-exported by `crate::sgi`
//
// These are used as self-documenting `HashOptions` field values.
// ============================================================================
...
///
/// This handle borrows the SGI instance for the duration of the operation.
///
/// SGI singleton ownership is enforced by the `Sgi` constructor consuming the
/// Embassy `Peri` token.
pub struct DmaHasher<'a, 'd> {
    sgi_instance: &'a mut Sgi<'d>,
...
impl<'a, 'd> DmaHasher<'a, 'd> {
    /// Finish the operation and write the digest into `hash_result`. Async while it waits for DMA completion and SGI processing.
    pub async fn finalize(mut self, hash_result: &mut [u8]) -> Result<(), SGIError> {
        if let Some(transfer) = self.transfer.take() {
            transfer.await.map_err(|_| SGIError::DmaError)?;

            // Wait for the SGI engine to finish processing the DMA-fed blocks.
            // This uses the SGI operation-done interrupt instead of busy-spin polling.
            self.sgi_instance.wait_sha2_complete_irq().await?;
        }
        ...
        // Enable IRQ before starting the final hash operation.
        self.sgi_instance.enable_operation_done_interrupt();
        ...
        // Wait for completion via SGI IRQ, then read digest.
        self.sgi_instance.wait_sha2_complete_irq().await?;
...
pub fn hash_dma_start<'a, 'd>(
    ...
    if dma_len_bytes > 0 {
        ...
        // The completion will be awaited in `DmaHasher::finalize`.
        sgi.enable_operation_done_interrupt();
        sgi.start_sgi_hash(options, &input[..dma_len_bytes])?;
        let transfer = sgi.fill_sha2_fifo_dma_start(dma_ch, &input[..dma_len_bytes], dma_len_bytes)?;
...
/// Blocking hash function for input data of any size up to 512 bytes. This internally handles padding and block processing, and returns the final hash output in `hash_result`.
pub fn hash_blocking<'d>(
    sgi: &mut Sgi<'d>,
    hash_size: HashSize,
    input: &[u8],
    hash_result: &mut [u8],
    ...
    #[cfg(feature = "defmt")]
    defmt::info!("Padded message length: {=usize}", len);
...
fn process_multi_block_update<'d>(hasher: &mut SGIHasher, sgi: &mut Sgi<'d>, input: &[u8]) -> Result<(), SGIError> {
...
fn process_single_block_update<'d>(hasher: &mut SGIHasher, sgi: &mut Sgi<'d>, input: &[u8]) -> Result<(), SGIError> {
...
impl SGIHasher {
    ...
    /// Initialize the hasher with the specified hash size and mode. This must be called before any update() calls.
    /// Leave option as None (unless there are very specific performance/memory concerns).
    ...
    pub fn update<'d>(&mut self, sgi: &mut Sgi<'d>, input: &[u8]) -> Result<(), SGIError> {
        let input_len = input.len() as usize;
        ...
        if input_len > MAX_BLOCK_SIZE {
            ...
            process_multi_block_update(self, sgi, &copy_buffer[..copy_len])?;
        ...
        process_single_block_update(self, sgi, input)
    }
    ...
    pub fn finalize<'d>(&mut self, sgi: &mut Sgi<'d>, hash_result: &mut [u8]) -> Result<(), SGIError> {
...
/// The following is a wrapper for SHA-512 ROTKH calculation which always returns 48 bytes by truncating the full SHA-512 digest.
/// Hardware SHA-512 RKTH - always returns 48 bytes for RKTH calculation
/// Hardware SHA-512 RKTH - always returns 48 bytes for RKTH calculation.
pub fn hsm_sha512_rkth<'d>(sgi: &mut Sgi<'d>, input: &[u8]) -> Option<[u8; 48]> {
    let mut full_digest = [0u8; 64]; // Buffer to hold the full SHA-512 digest

    if let Err(e) = hash_blocking(sgi, HashSize::Sha512, input, &mut full_digest) {
        let _ = &e;
        #[cfg(feature = "defmt")]
