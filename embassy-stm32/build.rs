...
fn main() {
    ...
    impl<'a> ClockGen<'a> {
        fn gen_clock(&mut self, peripheral: &str, name: &str) -> TokenStream {
            ...
            let msg = format!("peripheral '{peripheral}' is configured to use the '{clock_name}' clock, which is not running. Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock");
            quote!( unsafe { crate::rcc::get_freqs().#clock_name.expect(#msg) } )
        }
