...
fn main() {
    ...
    // ========
    // Generate DMA IRQs with weak linkage support.
    //
    // We generate:
    // 1. Internal handler functions named `__embassy_dma_{IRQ}` that contain the actual logic
    // 2. A linker script with `PROVIDE({IRQ} = __embassy_dma_{IRQ})` for weak symbol support
    //
    // This allows users to define their own DMA interrupt handlers that will override
    // the embassy defaults, while still allowing embassy's handlers to be used when
    // no user handler is defined.

    let dma_irqs_tokens: TokenStream = dma_irqs
        .iter()
        .map(|(irq, channels)| {
            let internal_handler = format_ident!("__embassy_dma_{}", irq);

            let channels_idents: Vec<_> = channels.iter().map(|c| format_ident!("{}", c)).collect();

            quote! {
                #[cfg(feature = "rt")]
                #[unsafe(no_mangle)]
                unsafe extern "C" fn #internal_handler() {
                    #(
                        <crate::peripherals::#channels_idents as crate::dma::ChannelInterrupt>::on_irq();
                    )*
                }
            }
        })
    ...
    g.extend(dma_irqs_tokens);

    // Generate linker script for DMA IRQ weak symbols
    gen_dma_irq_linker_script(&dma_irqs);
...
fn gen_memory_x(memory: &[MemoryRegion], out_dir: &Path) {
    ...
    // Include DMA IRQ weak symbols linker script
    writeln!(memory_x).unwrap();
    writeln!(memory_x, "INCLUDE dma_irq.x;").unwrap();
...
/// Generate linker script for DMA IRQ weak symbols.
///
/// This creates a `dma_irq.x` file with PROVIDE directives that allow user code
/// to override DMA interrupt handlers while providing embassy's handlers as defaults.
///
/// Example output:
/// ```text
/// PROVIDE(GPDMA1_CHANNEL0 = __embassy_dma_GPDMA1_CHANNEL0);
/// PROVIDE(GPDMA1_CHANNEL1 = __embassy_dma_GPDMA1_CHANNEL1);
/// ```
fn gen_dma_irq_linker_script(dma_irqs: &BTreeMap<&str, Vec<String>>) {
    let out_dir = PathBuf::from(env::var_os("OUT_DIR").unwrap());
    let mut content = String::new();

    writeln!(
        &mut content,
        "/* Embassy DMA interrupt weak symbols - auto-generated by build.rs */"
    )
    .unwrap();
    writeln!(
        &mut content,
        "/* User-defined handlers with the same name will override these defaults */"
    )
    .unwrap();
    writeln!(&mut content).unwrap();

    for irq in dma_irqs.keys() {
        writeln!(&mut content, "PROVIDE({irq} = __embassy_dma_{irq});").unwrap();
    }

    let out_file = out_dir.join("dma_irq.x");
    fs::write(&out_file, content).unwrap();

    // Tell cargo to include this directory in the linker search path
    println!("cargo:rustc-link-search={}", out_dir.display());
}
