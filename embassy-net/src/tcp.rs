...
use sporktcp::socket::tcp;
pub use sporktcp::socket::tcp::State;
use sporktcp::wire::{IpEndpoint, IpListenEndpoint};

use crate::time::duration_to_sporktcp;
...
impl<'a> TcpSocket<'a> {
    ...
    pub fn set_timeout(&mut self, duration: Option<Duration>) {
        self.io
            .with_mut(|s, _| s.set_timeout(duration.map(duration_to_sporktcp)))
    }
    ...
    pub fn set_keep_alive(&mut self, interval: Option<Duration>) {
        self.io
            .with_mut(|s, _| s.set_keep_alive(interval.map(duration_to_sporktcp)))
    }
...
impl<'d> TcpIo<'d> {
    ...
    fn read<'s>(&'s mut self, buf: &'s mut [u8]) -> impl Future<Output = Result<usize, Error>> + 's {
        poll_fn(|cx| {
            // CAUTION: sporktcp semantics around EOF are different to what you'd expect
    ...
    fn flush(&mut self) -> impl Future<Output = Result<(), Error>> + '_ {
        poll_fn(|cx| {
            self.with_mut(|s, _| {
                ...
                // sporktcp issues wake-ups when octets are dequeued from the send buffer
                if data_pending || fin_pending || rst_pending {
                    s.register_send_waker(cx.waker());
                    Poll::Pending
