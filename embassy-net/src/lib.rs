...
impl Inner {
    ...
    fn poll<D: Driver>(&mut self, cx: &mut Context<'_>, driver: &mut D) {
        ...
        if self.slaac && self.iface.slaac_updated_at() == timestamp {
            let ipv6_address = self.iface.ip_addrs().iter().find_map(|addr| {
                if let IpCidr::Ipv6(ip6_address) = addr
                    && !Ipv6Cidr::LINK_LOCAL_PREFIX.contains_addr(&ip6_address.address())
                {
                    return Some(ip6_address);
                }
                None
            });
            self.static_v6 = if let Some(address) = ipv6_address {
                let gateway = self
                    ...
                    .map(|r| {
                        if let IpAddress::Ipv6(gateway) = r.via_router {
                            return Some(gateway);
                        }
                        None
                    })
