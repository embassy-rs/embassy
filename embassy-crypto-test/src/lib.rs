    ...
    run(
        ...
        |case| {
            ...
            let accepted_prk = HkdfSha256::from_prk(&prk)
                ...
                .expand(case.info, b)
                .is_ok();
            if accepted != accepted_prk || a != b {
                return Err("new and from_prk disagree");
            }

            let verdict = judge(case.result, accepted)?;
            if accepted && *a != *case.okm {
...
macro_rules! curve_suites {
    ($mod:ident, $n:literal, $doc:literal) => {
        ...
        pub mod $mod {
            ...
            /// A point from a test vector: an uncompressed SEC1 encoding, or
            /// nothing for the point at infinity.
            fn point(bytes: &[u8]) -> Result<Point, &'static str> {
                if bytes.is_empty() {
                    return Ok(Point::identity());
                }
                let sec1: &[u8; 2 * N + 1] = bytes.try_into().map_err(|_| "bad point length in vector")?;
                Point::from_sec1(sec1).map_err(|_| "vector point rejected")
            }
            ...
            pub fn arith(suite: &Suite<EcArith>) -> Outcome {
                run(
                    ...
                    |case| {
                        ...
                        let p = point(case.a_g)?;
                        let q = point(case.b_g)?;
                        if Point::mul_base(&a) != p {
                        ...
                        let expected = point(case.lincomb)?;
                        if Point::lincomb(&a, &g, &b, &q) != expected {
                            return Err("linear combination mismatch");
                        }
                        if Point::lincomb_vartime(&a, &g, &b, &q) != expected {
                            return Err("variable-time linear combination mismatch");
                        }
                        // The same sum with a negated term, and as separate operations.
                        let minus_b = Scalar::ZERO.sub(&b);
                        let minus_q = q.neg();
                        if Point::lincomb(&a, &g, &minus_b, &minus_q) != expected {
                            return Err("negated linear combination mismatch");
                        }
                        if Point::lincomb_vartime(&a, &g, &minus_b, &minus_q) != expected {
                            return Err("negated variable-time linear combination mismatch");
                        }
                        if g.mul(&a).add(&q.mul(&b)) != expected {
                            return Err("linear combination from separate operations mismatch");
                        }
                        let (Some(q_aff), Some(minus_q_aff)) = (q.to_affine(), minus_q.to_affine()) else {
                            return Err("negation of a finite point is infinity");
                        };
                        if minus_q_aff.x != q_aff.x || !minus_q.add(&q).is_identity() || g.mul(&minus_b) != minus_q {
                            return Err("point negation mismatch");
                        }
                        let Some(p_aff) = p.to_affine() else {
                            return Err("finite point converts to infinity");
                        };
                        if p.to_sec1().as_ref().map(|s| &s[..]) != Some(case.a_g)
                            || Point::from_xy(&p_aff.x, &p_aff.y) != Ok(p)
                        {
                ...
                let g = Point::generator();
                let g_aff = g.to_affine().ok_or(Failure {
                    suite: suite_name,
                    tc_id: u32::MAX,
                    what: "generator is infinity",
                })?;
                check(
                    Point::from_xy(&g_aff.x, &g_aff.y) == Ok(g),
                    "generator not on curve",
                )?;
                let mut y = g_aff.y;
                y[N - 1] ^= 1;
                check(
                    Point::from_xy(&g_aff.x, &y) == Err(Error::InvalidKey),
                    "off-curve point accepted",
                )?;
                let mut sec1 = g.to_sec1().unwrap();
                sec1[0] = 0x02;
                ...
                check(inf.is_identity() && !g.is_identity(), "is_identity mismatch")?;
                check(
                    inf.to_affine().is_none() && inf.to_sec1().is_none(),
                    "infinity has coordinates",
                )?;
                check(
                    inf == Point::identity() && inf != g,
                    "infinity comparison mismatch",
                )?;
                check(
                    Point::mul_base(&Scalar::ZERO).is_identity(),
                    "0 * G is not infinity",
                )?;
                check(g.mul(&Scalar::ZERO).is_identity(), "0 * P is not infinity")?;
                check(inf.add(&g) == g && g.add(&inf) == g, "P + infinity is not P")?;
                check(inf.add(&inf).is_identity(), "infinity + infinity is not infinity")?;
                check(inf.neg().is_identity(), "-infinity is not infinity")?;
                ...
                check(inf.mul(&one).is_identity(), "1 * infinity is not infinity")?;
                check(Point::mul_base(&one) == g, "1 * G is not G")?;
                check(g.mul(&one) == g, "1 * P is not P")?;
                let n_minus_1 = Scalar::ZERO.sub(&one);
                let minus_g = g.mul(&n_minus_1);
                check(!minus_g.is_identity(), "(n-1) * G is infinity")?;
                check(g.add(&minus_g).is_identity(), "G + (-G) is not infinity")?;
                check(g.add(&g) == Point::mul_base(&two), "G + G != 2G")?;
                check(g.add(&g).add(&g) == Point::mul_base(&three), "G + G + G != 3G")?;
                check(g.neg() == minus_g, "-G != (n-1) * G")?;
                check(g.neg().neg() == g, "--G != G")?;
                check(
                    Point::lincomb(&one, &g, &n_minus_1, &g).is_identity(),
                    "G - G through lincomb is not infinity",
                )?;
                check(
                    Point::lincomb_vartime(&one, &g, &n_minus_1, &g).is_identity(),
                    "G - G through lincomb_vartime is not infinity",
                )?;
                check(
                    Point::lincomb(&Scalar::ZERO, &g, &Scalar::ZERO, &g).is_identity(),
                    "0 * G + 0 * G through lincomb is not infinity",
                )?;
                check(
                    Point::lincomb_vartime(&Scalar::ZERO, &g, &Scalar::ZERO, &g).is_identity(),
                    "0 * G + 0 * G through lincomb_vartime is not infinity",
                )?;
                check(
                    Point::lincomb(&one, &g, &Scalar::ZERO, &g) == g
                        && Point::lincomb(&Scalar::ZERO, &g, &one, &g) == g,
                    "one-sided lincomb is not 1 * G",
                )?;
                check(
                    Point::lincomb_vartime(&one, &g, &Scalar::ZERO, &g) == g
                        && Point::lincomb_vartime(&Scalar::ZERO, &g, &one, &g) == g,
                    "one-sided lincomb_vartime is not 1 * G",
                )?;
                check(
                    Point::lincomb(&one, &inf, &one, &g) == g && Point::lincomb_vartime(&one, &g, &one, &inf) == g,
                    "lincomb with an infinity term is not the other term",
                )?;
                check(
                    Point::lincomb(&two, &g, &one, &g) == Point::mul_base(&three),
                    "2G + G through lincomb is not 3G",
                )?;
                check(
                    Point::lincomb_vartime(&one, &g, &two, &g) == Point::mul_base(&three),
                    "G + 2G through lincomb_vartime is not 3G",
                )?;
                Ok(Stats {
            ...
            pub fn ecdsa(suite: &Suite<Ecdsa>) -> Outcome {
                ...
                let sk = SigningKey::generate().map_err(|_| fail("key generation failed"))?;
                let vk = sk
                    .verifying_key()
                ...
                let sig = sk.sign_prehash(digest).map_err(|_| fail("signing failed"))?;
                vk.verify_prehash(digest, &sig)
                    .map_err(|_| fail("own signature does not verify"))?;
                // Signatures are randomized: a second one differs and also verifies.
                let sig2 = sk.sign_prehash(digest).map_err(|_| fail("signing failed"))?;
                if sig2 == sig {
...
pub mod x25519 {
    ...
    pub fn keygen(suite: &Suite<KeyPair>) -> Outcome {
        ...
        let a = SecretKey::generate().map_err(|_| fail("key generation failed"))?;
        let b = SecretKey::generate().map_err(|_| fail("key generation failed"))?;
...
pub mod ed25519 {
    ...
    /// Run the verification suite. Signatures of the wrong length are
    /// rejected by the API's fixed-size parsing and count as rejected.
    pub fn verify(suite: &Suite<Eddsa>) -> Outcome {
        run(
            suite,
            |_, c| c.tc_id,
            |case| {
                ...
                let Ok(sig) = <&[u8; 64]>::try_from(case.sig) else {
                    return judge(case.result, false);
                };
                ...
                judge(case.result, vk.verify(case.msg, &sig).is_ok())
            },
        )
    }

    /// Run the signing suite: key derivation and deterministic signatures
    /// against the vectors, plus a sign-and-verify round trip with a fresh key.
    pub fn sign(suite: &Suite<Sign>) -> Outcome {
        let stats = run(
            suite,
            |i, _| i as u32,
            |case| {
                let private: &[u8; 32] = case
                    ...
                    .try_into()
                    .map_err(|_| "bad private key length in vector")?;
                let sk = SigningKey::from_bytes(private);
                if sk.to_bytes() != *private {
                    return Err("signing key round trip mismatch");
                }
                let vk = sk.verifying_key().map_err(|_| "verifying key derivation failed")?;
                if vk.as_bytes() != case.public {
                    return Err("verifying key mismatch");
                }
                let msg = &MESSAGE[..case.msg_len];
                let sig = sk.sign(msg).map_err(|_| "signing failed")?;
                if sig.as_bytes() != case.sig {
                    return Err("signature mismatch");
                }
                vk.verify(msg, &sig).map_err(|_| "own signature does not verify")?;
                Ok(Verdict::Pass)
            },
        )?;
        ...
        let vk = sk
            ...
            .map_err(|_| fail("verifying key derivation failed"))?;
        let msg = &MESSAGE[..100];
        let sig = sk.sign(msg).map_err(|_| fail("signing failed"))?;
        vk.verify(msg, &sig)
            .map_err(|_| fail("own signature does not verify"))?;
        let sig2 = sk.sign(msg).map_err(|_| fail("signing failed"))?;
        if sig2 != sig {
            return Err(fail("signatures are not deterministic"));
        }
        if vk.verify(&MESSAGE[..101], &sig) != Err(Error::InvalidSignature) {
            return Err(fail("signature verifies for another message"));
        }
        let mut bytes = sig.to_bytes();
        bytes[40] ^= 1;
        if vk.verify(msg, &Signature::from_bytes(&bytes)) != Err(Error::InvalidSignature) {
            return Err(fail("modified signature verifies"));
        }
        let other = SigningKey::generate().map_err(|_| fail("key generation failed"))?;
        let other_vk = other
            .verifying_key()
            .map_err(|_| fail("verifying key derivation failed"))?;
        if other_vk.verify(msg, &sig) != Err(Error::InvalidSignature) {
            return Err(fail("signature verifies under another key"));
        }
        // Not a point on the curve: for y = 2, x^2 = (y^2 - 1) / (d y^2 + 1) is not a square.
        let mut bad = [0u8; 32];
        bad[0] = 2;
        if VerifyingKey::from_bytes(&bad).verify(msg, &sig) != Err(Error::InvalidKey) {
            return Err(fail("undecodable verifying key accepted"));
        }
        Ok(stats)
    }
}
...
named! {
    ...
    /// Ed25519 verification (Wycheproof `ed25519_test`).
    ed25519_verify => ed25519::verify(&ED25519);
    /// Ed25519 key derivation and signing (generated).
    ed25519_sign => ed25519::sign(&ED25519_SIGN);
}
