...
/// `HAL_GPU2D_ErrorCallback()` and the cache-hold handshake stay dead.
const GPU2D_SYS_INTERRUPT_ENABLE_BUS_ERROR: u32 = 0x7E;
...
impl NeoChrom {
    ...
    pub fn new() -> Result<Self, InitError> {
        crate::hal::gpu2d_init_stub().map_err(|_| InitError::Gpu2d)?;
        Self::finish_init()
    }
    ...
    fn finish_init() -> Result<Self, InitError> {
        // Do NOT call `nema_sys_init()` as well. It is the platform hook that
        // `nema_init()` invokes internally, so calling it here creates the
        // submission ring buffer twice; the duplicate showed up as the ring
        // registers moving by exactly RING_SIZE (0x341c01fc -> 0x341c05fc).
