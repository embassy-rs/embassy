...
fn dcache_maintain(addr: usize, len: usize, op: MaintainOp) {
    #[cfg(target_arch = "arm")]
    {
        ...
        // by `steal()` is the usual way to reach it outside `embassy-stm32`. Mirrors
        // `crate::hal::nema_gfx_hal_dcache_clean_invalidate`.
        let mut scb = unsafe { cortex_m::peripheral::Peripherals::steal().SCB };
        match op {
            MaintainOp::Clean => scb.clean_dcache_by_address(addr, len),
            MaintainOp::CleanInvalidate => scb.clean_invalidate_dcache_by_address(addr, len),
        }
    }
