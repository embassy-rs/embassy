...
impl<'d, T: Instance> Spi<'d, T, Async> {
    ...
    async fn transfer_inner(&mut self, rx: *mut [u8], tx: *const [u8]) -> Result<(), Error> {
        ...
        let tx_transfer = async {
            let p = self.inner.regs();
            unsafe {
                crate::dma::write(tx_ch.reborrow(), tx, p.dr().as_ptr() as *mut _, T::TX_DREQ).wait().await;

                if rx.len() > tx.len() {
                    let write_bytes_len = rx.len() - tx.len();
                    // write dummy data
                    // this will disable incrementation of the buffers
                    crate::dma::write_repeated(tx_ch, p.dr().as_ptr() as *mut u8, write_bytes_len, T::TX_DREQ).wait().await
                }
            }
        };
