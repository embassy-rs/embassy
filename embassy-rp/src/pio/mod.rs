...
impl<'d, PIO: Instance, const SM: usize> StateMachineRx<'d, PIO, SM> {
    ...
    pub fn dma_pull<'a, C: Channel, W: Word>(
        ...
        unsafe { crate::dma::read(ch, PIO::PIO.rxf(SM).as_ptr() as *const W, data, Self::dreq(), bswap) }
    }
    ...
    pub fn dma_pull_repeated<'a, C: Channel, W: Word>(&'a mut self, ch: Peri<'a, C>, len: usize) -> Transfer<'a, C> {
        unsafe { crate::dma::read_repeated(ch, PIO::PIO.rxf(SM).as_ptr(), len, Self::dreq()) }
    }
}
...
impl<'d, PIO: Instance, const SM: usize> StateMachineTx<'d, PIO, SM> {
    ...
    pub fn dma_push<'a, C: Channel, W: Word>(
        ...
        unsafe { dma::write(ch, data, PIO::PIO.txf(SM).as_ptr() as *mut W, Self::dreq(), bswap) }
    }
