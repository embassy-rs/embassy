...
pub enum StepPulseTiming {
    /// 32 cycles high + 32 cycles low + 2 jump cycles = 66 total.
    Cycles66,
    /// 64 cycles high + 64 cycles low + 2 jump cycles = 130 total.
    Cycles130,
    /// 128 cycles high + 128 cycles low + 2 jump cycles = 258 total.
    Cycles258,
}

impl Default for StepPulseTiming {
    fn default() -> Self {
        StepPulseTiming::Cycles66
    }
}
...
impl StepPulseTiming {
    fn cycles_per_pulse(self) -> u32 {
        match self {
            StepPulseTiming::Cycles66 => 66,
            StepPulseTiming::Cycles130 => 130,
            StepPulseTiming::Cycles258 => 258,
        }
    }
}

/// Calculate the minimum pulse frequency in Hz that can be generated by the PIO program given the system clock frequency.
pub fn min_frequency_hz(cycles_per_pulse: u32) -> u32 {
...
impl<'d, T: Instance, const SM: usize> PioStepDir<'d, T, SM> {
    ...
    pub fn min_frequency_hz(&self) -> u32 {
        min_frequency_hz(self.cycles_per_pulse)
    }

    /// Set output pulse frequency in pulses per second.
    pub fn set_frequency(&mut self, freq_hz: u32) -> Result<(), StepDirFrequencyError> {
        if freq_hz == 0 {
            return Err(StepDirFrequencyError::Zero);
        }
        let min_freq = min_frequency_hz(self.cycles_per_pulse);
        if freq_hz < min_freq {
    ...
    /// Get the minimum achievable pulse frequency in Hz for the current system clock.
    ...
    /// Start continuous pulse generation using the currently set direction.
    ///
    /// This runs until [`stop_continuous`] is called.
    pub fn start_continuous(&mut self) {
        if self.running_continuous {
            return;
        }
        self.running_continuous = true;
        self.sm.tx().push(u32::MAX);
    }

    /// Start continuous pulse generation in the given direction.
    ///
    /// This runs until [`stop_continuous`] is called.
    pub fn start_continuous_with_dir(&mut self, direction: StepDir) {
        self.set_direction(direction);
        self.start_continuous();
    }

    /// Stop continuous pulse generation.
    ...
    /// Returns true if continuous mode has been started and not yet stopped.
    pub fn is_running_continuous(&self) -> bool {
        self.running_continuous
    }
