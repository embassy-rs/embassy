...
//!
//! Unlike the CryptoCell PKA, this engine implements whole public key operations in hardware:
//! software writes the operands into a dedicated crypto RAM, names an operation, and reads the
//! result back. The crypto RAM is divided into fixed-size slots, and every operation has a
//! fixed assignment of operands to slots. In big-endian mode, which every operation here uses,
//! an operand is right-aligned at the end of its slot.
//!
//! The operation codes, slot assignments and status codes follow the `silexpk` layer of
//! Nordic's `nrf_security`.
...
fn slot_size() -> usize {
    if pk().hwconfig().read().maxopsize() > 0x200 { 0x400 } else { 0x200 }
}
...
impl Handle {
    pub(super) fn new(microcode: &[u32]) -> Self {
        let activation = crate::cracen::activate();
        // The microcode RAM loses its contents whenever CRACEN is powered down, and this
        // driver holds it up from here on.
...
/// Writes a big-endian value right-aligned in the operand window of a slot, zeroing the rest
/// of the window.
///
/// The crypto RAM only accepts whole aligned words, so the value is composed into words. The
/// bytes of the slot below the operand window are not part of any operand; zeroing a few of
/// them to keep the writes aligned is harmless.
...
struct Op {
    ...
    ptr_a: u8,
    /// Countermeasures against power analysis, which need a random factor in
    /// [`SLOT_BLIND`].
    blind: Blind,
}
...
impl Op {
    ...
    /// Draws the random factor the countermeasures multiply with.
    ///
    /// The engine needs an odd factor of a fixed width for its running time not to depend on
    /// it, which is what the forced bits are for.
    fn write_blinding_factor(&self) {
        let mut words = [0u32; BLIND_LEN / 4];
        crate::cracen::random_words(&mut words);
...
fn write_curve(curve: &Curve) {
    ...
    let n = curve.size();
    for (i, param) in [curve.p, curve.n, curve.gx, curve.gy, curve.a, curve.b].iter().enumerate() {
...
pub(super) fn ecc_mul(
    ...
    let op = ecc_op(OP_ECC_PTMUL, curve, SLOT_PX, Blind::Ecc);
    op.start()?;
    write_curve(curve);
