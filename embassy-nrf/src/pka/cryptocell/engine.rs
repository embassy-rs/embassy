...
//! operation is issued by writing one opcode word naming the two operand registers, the result
//! register, and one of eight operand sizes held in the sizes table; the engine then runs it
//! over the whole register width. Modular operations reduce with Barrett's algorithm, which
//! needs the modulus in register `N` and its Barrett tag in register `NP`.
//!
//! The register file is virtual: a mapping table gives the SRAM address of each register, and
//! the engine reserves two of them (30 and 31) as its own temporaries.
//!
...
///
/// Both CryptoCell-310 and CryptoCell-312 have a 64x16 multiplier. The width sets the
/// granularity of every register and the size of the Barrett tags.
...
///
/// The mapping table is laid out within this, so it bounds how many registers an operation of
/// a given size can use.
...
op_ri!(
    /// `res = (a + imm) mod n`
    mod_add_im, ModAddInc
);
op_rr!(
    /// `res = (a - b) mod n`
    mod_sub, ModSubDecNeg
);
op_ri!(
    /// `res = a & imm`
    and_im, Andtst0clr0
...
///
/// The shift count goes in the second operand field, but unlike a real immediate operand it
...
/// Writes one 32-bit word of a register.
///
/// The write is only committed once the whole PKA word holding it has been written, so this
/// rewrites the words around it, which must be zero.
...
///
/// Lays out the register mapping table so that as many registers as fit in the SRAM are
/// usable, and fills the sizes table. `regs_needed` counts the registers the operation uses
/// plus the two the engine reserves for itself; if that many do not fit, the modulus is too
/// large for the operation and this fails.
...
///
/// The tag is `floor(2^(N + A + X - 1) / n)` for a modulus of `size_bits` bits, where `A` is
/// the PKA word size and `X` the extra bits; for a modulus larger than `2·(A + X)` the
/// division is done on a truncated modulus, which is what the engine expects.
