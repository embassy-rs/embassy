...
use crate::cracen::cmdma::{self, Chain, ENGINE_AES, REALIGN, tag_config, tag_data, tag_ignore};
...
/// Loads a fresh random mask into the AES engine for its side-channel countermeasures, as
/// NCS does before AES operations.
fn load_countermeasure_mask() {
    let mask = crate::cracen::random_word();
...
/// GCM state: the engine context (saved between transactions), or the IV before the first
/// transaction.
...
/// One GCM transaction: optional AAD, optional data, and optionally the final length block
/// producing the tag.
///
