...
//!
//! This driver uses the AES engine of the CryptoCell subsystem (nRF52840, nRF91, nRF5340) or
//! of the CRACEN CryptoMaster (nRF54L).
//!
...
//! # Usage
//!
//! A cipher operation is started with [`Aes::start`], which returns a [`Context`] holding the
//! state of that operation. Data is then processed with [`Aes::blocking_aad`] (authenticated
//! modes only) and [`Aes::blocking_payload`], and the operation is completed with
//! [`Aes::blocking_finish`], which returns the authentication tag for MAC and AEAD modes.
//! Any number of contexts may be interleaved on the same driver.
//!
...
pub enum Error {
    ...
    ///
    /// Returned when the input and output lengths differ, when a chunk that is not the last
    /// one is not a multiple of the block length (for modes that require it), or when the
    /// amount of data does not match the lengths declared when creating an [`AesCcm`] cipher.
    InvalidLength,
...
/// AES cipher mode that authenticates additional data (AEAD).
#[allow(private_bounds)]
pub trait AuthenticatedCipher: Cipher {}

/// AES in ECB (electronic codebook) mode.
///
/// Data must be a multiple of the block length. ECB does not hide data patterns and should
/// not be used to encrypt data directly.
#[derive(Clone, Copy)]
pub struct AesEcb {
    key: Key,
}
...
impl AesEcb {
    /// Creates an ECB cipher with the given key (16, 24 or 32 bytes, depending on hardware support).
...
///
/// Data must be a multiple of the block length. The IV must be unpredictable and unique per
/// message.
#[derive(Clone, Copy)]
pub struct AesCbc {
    key: Key,
...
impl AesCbc {
    /// Creates a CBC cipher with the given key and IV.
...
///
/// The 16-byte initial counter block is treated as a big-endian integer that is incremented
/// for every block (NIST SP 800-38A). Data may have any length. Encryption and decryption
/// are the same operation. The initial counter must never repeat for the same key.
#[derive(Clone, Copy)]
pub struct AesCtr {
    key: Key,
    iv: [u8; BLOCK_LEN],
...
/// AES-CMAC message authentication code (NIST SP 800-38B).
///
/// The message is fed with [`Aes::blocking_payload`] (the output buffer is ignored and may
/// be empty). The 16-byte tag is returned by [`Aes::blocking_finish`]. The direction passed
/// to [`Aes::start`] is ignored.
#[derive(Clone, Copy)]
pub struct AesCmac {
    key: Key,
}
...
impl AesCmac {
    /// Creates a CMAC with the given key.
    pub fn new(key: &[u8]) -> Result<Self, Error> {
        Ok(Self { key: Key::new(key)? })
    }
}
...
/// AES in CCM (counter with CBC-MAC) authenticated mode (NIST SP 800-38C).
///
/// CCM needs to know the amount of additional authenticated data and payload up front. The
/// tag returned by [`Aes::blocking_finish`] is valid in its first `tag_len` bytes.
#[derive(Clone, Copy)]
pub struct AesCcm {
    key: Key,
    nonce: [u8; 13],
...
impl AesCcm {
    ...
    ///
    /// - `nonce`: 7 to 13 bytes, unique per message.
...
/// AES in GCM (Galois/counter) authenticated mode (NIST SP 800-38D).
///
/// Only 96-bit (12 byte) IVs are supported. The IV must be unique per message: reuse is
/// catastrophic. The tag returned by [`Aes::blocking_finish`] is 16 bytes.
#[cfg(any(feature = "_cryptocell-312", feature = "_cracen"))]
#[derive(Clone, Copy)]
pub struct AesGcm {
    key: Key,
...
#[cfg(any(feature = "_cryptocell-312", feature = "_cracen"))]
impl AesGcm {
    /// Creates a GCM cipher with the given key and 96-bit IV.
...
impl MacAcc {
    ...
    ///
    /// With `hold_back`, the last block is never processed until [`Self::flush`] is called, so
    /// that `process` can be told which block is the final one (CMAC).
...
/// State of an in-progress AES operation, created by [`Aes::start`].
#[derive(Clone)]
pub struct Context<C: Cipher> {
    cipher: C,
    dir: Direction,
    phase: Phase,
    /// CBC chaining value.
...
/// AES driver.
pub struct Aes<'d, M: Mode> {
    _hw: hw::Handle,
    _phantom: PhantomData<(&'d (), M)>,
}

impl<'d> Aes<'d, Blocking> {
    /// Creates a new blocking AES driver.
    pub fn new_blocking(_peri: Peri<'d, peripherals::AES>) -> Self {
        Self {
            _hw: hw::Handle::new(),
...
impl<'d, M: Mode> Aes<'d, M> {
    /// Starts a cipher operation.
    pub fn start<C: Cipher>(&mut self, cipher: C, dir: Direction) -> Context<C> {
        let key = *cipher.key();
        let mut ctx = Context {
            cipher,
            dir,
    ...
    ///
    /// Must be called before any payload is processed. AAD may be fed in chunks of any size.
    /// `last` marks the final chunk; it is optional, the AAD phase also ends when the first
    /// payload chunk is fed or the operation is finished.
    pub fn blocking_aad<C: AuthenticatedCipher>(
        &mut self,
        ctx: &mut Context<C>,
        aad: &[u8],
        last: bool,
    ...
    fn end_aad<C: Cipher>(ctx: &mut Context<C>) -> Result<(), Error> {
        let key = *ctx.cipher.key();
        match ctx.cipher.params() {
            Params::Ccm { aad_len, .. } => {
    ...
    ///
    /// `input` and `output` must have the same length. For [`AesCmac`], `output` is ignored.
    ///
    /// `last` marks the final chunk of payload. Chunks that are not the last one must be a
    /// multiple of the block length for ECB, CBC and GCM. ECB and CBC require every chunk to
    /// be a multiple of the block length. CTR, CMAC and CCM accept chunks of any length.
    pub fn blocking_payload<C: Cipher>(
        &mut self,
        ctx: &mut Context<C>,
        input: &[u8],
        output: &mut [u8],
        ...
        unsafe { Self::payload_raw(ctx, input.as_ptr(), output.as_mut_ptr(), input.len(), last) }
    }

    /// Processes payload data in place. See [`Self::blocking_payload`].
    pub fn blocking_payload_in_place<C: Cipher>(
        &mut self,
        ctx: &mut Context<C>,
        data: &mut [u8],
        last: bool,
    ) -> Result<(), Error> {
        unsafe { Self::payload_raw(ctx, data.as_ptr(), data.as_mut_ptr(), data.len(), last) }
    }
    ...
    /// `input` must be readable and `output` writable for `len` bytes. If they overlap, they
    /// must be equal.
    unsafe fn payload_raw<C: Cipher>(
        ctx: &mut Context<C>,
        input: *const u8,
        output: *mut u8,
        len: usize,
    ...
    /// `tag_len` bytes of it are valid. When decrypting, compare the returned tag against the
    /// received one in constant time before using the plaintext.
    pub fn blocking_finish<C: Cipher>(&mut self, mut ctx: Context<C>) -> Result<Option<[u8; BLOCK_LEN]>, Error> {
        let key = *ctx.cipher.key();
...
impl<'d, M: Mode> Drop for Aes<'d, M> {
    fn drop(&mut self) {
