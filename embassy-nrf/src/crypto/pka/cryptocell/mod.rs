...
pub(super) use ecc::ecc_lincomb;
pub(super) use ecc::{ecc_add, ecc_double, ecc_mul, ecdsa_sign, ecdsa_verify, point_check};
use engine::{LEN_FULL, LEN_MAX, LEN_N, LEN_PQ, REG_N, REG_NP, REG_T0, REG_T1};

use crate::crypto::pka::{Error, bit_len};
...
impl Handle {
    pub(super) fn new() -> Self {
        Self {
            _activation: crate::crypto::activate(),
        }
    }
}
...
///
/// Follows `RsaExecPrivKeyExpCrt` of Arm's CryptoCell runtime library. Each half-size
/// exponentiation needs its prime in the modulus register. So the second prime is moved into
/// a spare register, and the engine is pointed at it.
