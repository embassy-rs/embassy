...
//! `nrf_security`. The engine has no round-count setting: it only does ChaCha20, which is
//! the only [`ChaChaVariant`] on this chip.

use super::{BLOCK_LEN, ChaChaVariant, KEY_LEN, NONCE_LEN};
use crate::crypto::cracen::cmdma::{self, Chain, ENGINE_CHACHA, REALIGN, tag_config, tag_data};
...
pub(super) unsafe fn apply(
    ...
    let ChaChaVariant::ChaCha20 = variant;
...
pub(super) mod aead {
    ...
    use super::super::{BLOCK_LEN, ChaChaVariant, Direction, Error, KEY_LEN, NONCE_LEN};
    use super::{
        CFG_CTX_LOAD, CFG_CTX_SAVE, CFG_DECRYPT, MODE_CHACHA20_POLY1305, TAG_AAD, TAG_CFG, TAG_COUNTER, TAG_DATA,
        TAG_KEY, TAG_NONCE,
    };
    use crate::crypto::cracen::cmdma::{self, Chain, REALIGN};
    ...
    impl ChaChaPolyContext {
        ...
        /// Runs one transaction: optional AAD (a multiple of 16 bytes), optional data, and
        /// with `finish` the final length block that produces the tag.
