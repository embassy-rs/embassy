...
//!
//! All engines feed from the single DIN DMA and write to the single DOUT DMA, so only one
//! operation can be in flight at a time. Every transaction is run inside a critical section
//! by the callers, and is bounded to [`MAX_CHUNK`] bytes to keep interrupt latency bounded.
...
/// Runs one transfer of `len` bytes from `input` through the engines, writing the engine
/// output to `output` if not null, and waits for it to complete.
///
/// # Safety
///
/// `input` must point to `len` readable bytes in RAM, and `output` (if not null) to `len`
/// writable bytes in RAM, for the duration of the call.
