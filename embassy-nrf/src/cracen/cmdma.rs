...
//!
//! Every symmetric operation on CRACEN is a scatter-gather DMA transaction: an input ("fetch")
//! chain of descriptors that carries engine configuration and data, and an output ("push")
//! chain that receives the results. Descriptor tags route each input descriptor to an engine
//! (BA411E AES, BA413 hash, BA417 ChaCha20-Poly1305) and describe its role.
//!
...
impl<const N: usize> Chain<N> {
    ...
    /// Adds an input descriptor of `len` bytes rounded up to a multiple of `align`, with the
    /// padding bytes marked as ignored. The engine reads the padding, so `addr` must have
    /// the rounded-up length readable.
...
/// writable RAM, for the whole length given (including padding), and the chains must not
/// be moved while the transaction runs.
