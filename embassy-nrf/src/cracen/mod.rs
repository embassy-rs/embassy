...
//!
//! This module contains the CRACEN random number generator. The symmetric engines of the
//! CryptoMaster are exposed by the [`aes`](crate::aes), [`hash`](crate::hash) and
//! [`chacha`](crate::chacha) modules.
...
/// whenever CRACEN is powered down. NCS programs them on every power-up in `cracen_acquire`,
/// calling it a workaround for incorrect hardware defaults on CRACEN Lite.
///
...
/// dropped.
pub(crate) fn activate() -> CracenActivationHandle {
    ...
    CracenActivationHandle { _private: () }
}
...
pub(crate) struct CracenActivationHandle {
    _private: (),
}

impl Drop for CracenActivationHandle {
    fn drop(&mut self) {
        release()
    }
}
...
/// Starts the TRNG. CRACEN must be powered.
fn start_rng() {
...
fn stop_rng() {
    // On CRACEN Lite the TRNG is only stopped by powering CRACEN down, which happens when
...
/// Fills `out` with random words, keeping the generator running for all of them rather than
/// paying its start-up time once per word.
