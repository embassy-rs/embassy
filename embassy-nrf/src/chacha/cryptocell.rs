...
pub(super) unsafe fn apply(
    ...
    critical_section::with(|_| {
        ...
        r.chacha_control().write(|w| {
            ...
            w.set_num_of_rounds(NumOfRounds::Default);
...
///
/// The CryptoCell has no Poly1305 engine: the message authenticator runs on the PKA, which
/// evaluates the polynomial over the field of `2¹³⁰ - 5` one block at a time.
pub(super) mod aead {
    use super::super::{BLOCK_LEN, Context, Direction, Error, KEY_LEN, NONCE_LEN};
    use crate::pka::Poly1305;
    ...
    impl AeadContext {
        ...
        pub(crate) unsafe fn payload(
            ...
            // The tag covers the ciphertext, which is the output when encrypting and the
            // input when decrypting.
            let ct = match self.dir {
                Direction::Encrypt => output as *const u8,
