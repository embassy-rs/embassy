...
#[repr(u16)]
pub enum Feature {
    #[doc = "2: Controller only, with standard feature set"]
    MasterOnly = 2,
    #[doc = "3: Controller and target, with standard feature set"]
    MasterAndSlave = 3,
}
...
impl FeatureR {
    ...
    pub const fn variant(&self) -> Option<Feature> {
        match self.bits {
            2 => Some(Feature::MasterOnly),
            3 => Some(Feature::MasterAndSlave),
            _ => None,
        }
    }
...
#[doc = "`reset()` method sets VERID to value 0x0103_0003"]
impl crate::Resettable for VeridSpec {
    const RESET_VALUE: u32 = 0x0103_0003;
}
