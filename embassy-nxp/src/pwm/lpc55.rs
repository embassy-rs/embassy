    ...
    fn configure(output_number: usize, config: &Config) {
        ...
        SCT0.ctrl().modify(|w| {
            if config.phase_correct {
                w.set_bidir(0, crate::pac::sct0::vals::Bidir::UP_DOWN);
            } else {
                w.set_bidir(0, crate::pac::sct0::vals::Bidir::UP);
            }
        ...
        critical_section::with(|_cs| {
            ...
            if SCT0.ev(0).ev_ctrl().read().matchsel() != 15 {
                SCT0.ev(0).ev_ctrl().modify(|w| {
                    ...
                    w.set_stateld(crate::pac::sct0::vals::Stateld::ADD);
                    w.set_statev(0);
                });
            }
        });
        ...
        // State 0 by default
        SCT0.state().modify(|w| w.set_state(0, 0));
        // Remove halt and start the actual counter
        SCT0.ctrl().modify(|w| {
            w.set_halt(0, !config.enable);
        });
    }
