...
async fn main(_spawner: Spawner) {
    ...
    let mut blocks = [Aligned([0u8; 512]), Aligned([0u8; 512])];
    {
        ...
        {
            let mut storage = loop {
                match BlockDevice::new_sd_card(&mut s, 24_000_000, Delay).await {
                    Ok(storage) => break storage,
                    Err(e) => match e {
                        MmcError::Timeout => defmt::panic!("card timeout!"),
                        _ => defmt::panic!("other card error!"),
                    },
                }
            };
