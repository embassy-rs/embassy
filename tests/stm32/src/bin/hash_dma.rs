...
async fn test_interrupt(hw_hasher: &mut Hash<'_, peripherals::HASH, Async>) {
    ...
    let _ = hw_hasher.finish(sha224context, &mut sha224_digest_buffer).await;

    // Finish the SHA-256 digest.
    hw_hasher.update(&mut sha256context, test_2).await;
    let mut sha256_digest_buffer: [u8; 32] = [0; 32];
    let _ = hw_hasher.finish(sha256context, &mut sha256_digest_buffer).await;
...
async fn test_sizes(hw_hasher: &mut Hash<'_, peripherals::HASH, Async>) {
    ...
    for i in 1..10 {
        ...
        for j in [1, 1, 2, 3, 4, 5, 123, 124, 125, 126, 127, 128, 129, 130, 131, 132, 133] {
            ...
            sw.update(&in1[..i]);
            sw.update(&in2[..j]);
            hw_hasher.update(&mut ctx, &in1[..i]).await;
            hw_hasher.update(&mut ctx, &in2[..j]).await;

            let sw_digest = sw.finalize();
            let mut hw_digest = [0u8; 64];
            hw_hasher.finish(ctx, &mut hw_digest).await;
    ...
    hw_hasher.update(&mut ctx, &boundary_input).await;
    let sw_digest = sw.finalize();
    let mut hw_digest = [0u8; 64];
    hw_hasher.finish(ctx, &mut hw_digest).await;
    defmt::assert!(hw_digest == *sw_digest);
...
async fn main(_spawner: Spawner) {
    ...
    let mut hw_hasher = Hash::new(p.HASH, p.GPDMA1_CH0, Irqs);
