...
use embassy_nrf::aes::{Aes, AesEcb, Direction};
use embassy_nrf::mode::Blocking;
use panic_probe as _;

#[cfg(feature = "cracen")]
type Rng<'d> = embassy_nrf::cracen::Cracen<'d, Blocking>;
#[cfg(not(feature = "cracen"))]
type Rng<'d> = embassy_nrf::cryptocell::rng::CcRng<'d, Blocking>;
...
async fn main(_spawner: Spawner) {
    ...
    defmt::unwrap!(aes.blocking_payload(&mut ctx, &block, &mut out, true));
    let first = out;
    check(&mut rng);
    ...
    drop(rng);
    let mut aes = Aes::new_blocking(unsafe { embassy_nrf::peripherals::AES::steal() });
    block[0] = 0;
    let mut ctx = aes.start(cipher, Direction::Encrypt);
    defmt::unwrap!(aes.blocking_payload(&mut ctx, &block, &mut out, true));
    assert!(out == first);
