...
async fn main(_spawner: Spawner) {
    ...
    // Test level-gated
    {
        ...
        let pwm = Pwm::new_input(&mut p.PWM_CH3, &mut p7, Pull::None, InputMode::Level, cfg.clone());
    ...
    {
        let mut pin2 = Output::new(&mut p11, Level::Low);
        let pwm = Pwm::new_input(&mut p.PWM_CH3, &mut p7, Pull::None, InputMode::RisingEdge, cfg.clone());
        assert_eq!(pwm.counter(), 0);
        Timer::after_millis(5).await;
        assert_eq!(pwm.counter(), 0);
        pin2.set_high();
