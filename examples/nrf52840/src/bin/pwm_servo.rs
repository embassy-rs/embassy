...
async fn main(_spawner: Spawner) {
    ...
    // set_frequency can only set down to 489hz with the default prescaler so we cant use it directly
    ...
    loop {
        ...
        pwm.ch0().set_duty(DutyCycle::normal(2500 - 156));
        Timer::after_millis(5000).await;

        info!("90 deg");
        pwm.ch0().set_duty(DutyCycle::normal(2500 - 187));
        Timer::after_millis(5000).await;

        info!("135 deg");
        pwm.ch0().set_duty(DutyCycle::normal(2500 - 218));
        Timer::after_millis(5000).await;

        info!("180 deg");
        pwm.ch0().set_duty(DutyCycle::normal(2500 - 250));
        Timer::after_millis(5000).await;

        info!("0 deg");
        pwm.ch0().set_duty(DutyCycle::normal(2500 - 125));
        Timer::after_millis(5000).await;
    }
}
