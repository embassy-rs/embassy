...
async fn main(_spawner: Spawner) -> ! {
    ...
    // The red and green part of the RGB LED of the LaunchPad.
    let red = PwmPin::<_, Ch0>::new(p.PA26, false);
    let green = PwmPin::<_, Ch1>::new(p.PA27, false);

    let mut config = PwmConfig::default();
    config.frequency = 10_000;

    let mut pwm = unwrap!(SimplePwm::new(p.TIMG1, Some(red), Some(green), None, None, config));
