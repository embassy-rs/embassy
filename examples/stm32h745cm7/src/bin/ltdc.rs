...
async fn main(spawner: Spawner) {
    ...
    // At a minimum we need to pull LCD_DISP (PD7) high and enable the LCD backlight (PK0).
    // PK7 (LCD_DISP_EN/LTDC_DE) is passed to the LTDC driver below.
    // The BSP code from ST also controls the reset pin (PA2), but in testing this did not
    // change anything.
    let _lcd_display_mode = Output::new(p.PD7, Level::High, Speed::Low);
    let _lcd_bl_ctrl = Output::new(p.PK0, Level::High, Speed::Low);
    // let _lcd_reset = Output::new(p.PA2, Level::High, Speed::Low);

    // Numbers from the STMicroelectronics STM32H745I-DISCO BSP.
    ...
    let mut ltdc = Ltdc::<_, ltdc::Rgb888>::new_with_pins(
        p.LTDC, Irqs, p.PI14, p.PI12, p.PI9, p.PK7, p.PJ12, p.PJ13, p.PJ14, p.PJ15, p.PK3, p.PK4, p.PK5, p.PK6, p.PJ7,
        p.PJ8, p.PJ9, p.PJ10, p.PJ11, p.PI0, p.PI1, p.PK2, p.PI15, p.PJ0, p.PJ1, p.PH9, p.PJ3, p.PJ4, p.PJ5, p.PJ6,
    );
