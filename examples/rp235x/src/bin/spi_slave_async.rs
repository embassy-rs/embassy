...
//! Note: By default the SPI slave on the RP235x requires CS to be toggled for each byte
//!
//!
...
//!
//! You should receive the tx in rx of the other.
//! Sample output:
...
async fn main(_spawner: Spawner) {
    ...
    let slave_rx = p.PIN_0;
    let slave_tx = p.PIN_3;
    let slave_clk = p.PIN_2;
    let slave_cs_pin = p.PIN_1;

    let master_spi = spi::Spi::new(
        p.SPI1,
        master_clk,
        master_mosi,
        master_miso,
    ...
    let slave_config = spi::Config::default();
    // If you need a single CS pulse for multiple bytes, uncomment the following:
    // slave_config.phase = embassy_rp::spi::Phase::CaptureOnSecondTransition;
    // slave_config.polarity = embassy_rp::spi::Polarity::IdleHigh;
    let slave_spi = spi::Spi::new_slave(
        p.SPI0,
        slave_clk,
        slave_tx,
        slave_rx,
