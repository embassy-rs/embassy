...
async fn main(_spawner: Spawner) {
    ...
    info!("streaming; B4 = save BMP, B2 = toggle auto WB");
    ...
    loop {
        ...
        .await
        {
            ...
            Either3::Third(()) => {
                if let Some((volume_mgr, next_idx)) = sd_state.as_mut() {
                    info!("save: capturing IMG{:04}.BMP", *next_idx);
                    save_current_frame(volume_mgr, &mut pipe1, last_idx, *next_idx).await;
