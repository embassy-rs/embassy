//! JSON-driven Rhai node (PoC for CANbossTouch) on the STM32H5F5J-DK — with FDCAN.
//!
...
//!   `"mode": "normal"` needs an external transceiver on the FDCAN1 pins
//!
//! Board (STM32H5F5J-DK):
//! NOTE: embassy-stm32 has no `stm32h5f5lj` support yet — this crate builds with
//! the `stm32h563zi` stand-in (see IDEAS.md), so the GPIO assignments below are
//! Nucleo-H563ZI placeholders until the real chip/pin support lands:
//! - LEDs (active-high placeholders): PB0 (green), PF4 (yellow), PG4 (red)
//! - user button PC13 (active-high, pull-down placeholder)
//! - FDCAN1 on PA11 (RX) / PA12 (TX) — Zephyr default, matches IDEAS.md
//! `"mode": "loopback"` (the default here) never touches the pins, so the
//! JSON/Rhai/OD/CAN logic is fully testable despite the stand-in chip.
...
/// Board (Nucleo-H563ZI placeholder pins): LEDs are active-high, the user
/// button is active-high (pull-down).
const LED_ACTIVE_LOW: bool = false;
const BUTTON_ACTIVE_LOW: bool = false;
...
async fn main(spawner: Spawner) {
    ...
    info!("json_node: JSON-driven Rhai node PoC (STM32H5F5J-DK, stand-in pins)");

    // Board I/O — Nucleo-H563ZI placeholder pins until H5F5 chip support lands:
    // LEDs PB0 (green), PF4 (yellow), PG4 (red) active-high, button PC13 pull-down.
    LEDS.lock(|cell| {
        let mut leds = cell.borrow_mut();
        leds.push(Output::new(p.PB0, Level::Low, Speed::Low));
        leds.push(Output::new(p.PF4, Level::Low, Speed::Low));
        leds.push(Output::new(p.PG4, Level::Low, Speed::Low));
    });
    BUTTON.lock(|cell| {
        cell.replace(Some(Input::new(p.PC13, Pull::Down)));
    });
