...
// STM32F0 uses combined I2C interrupts (not separate EV/ER like STM32F4)
...
async fn main(_spawner: Spawner) {
    ...
    info!("I2C1 (slave) on PB8/PB9, I2C2 (master) on PB10/PB11");
    info!("==============================================");
    ...
    let i2c_slave = I2c::new(p.I2C1, p.PB8, p.PB9, p.DMA1_CH2, p.DMA1_CH3, Irqs, i2c1_config)
        .into_slave_multimaster(SlaveAddrConfig::basic(I2C_ADDR));

    // I2C2 as master (PB10=SCL, PB11=SDA)
    // DMA: CH4=TX, CH5=RX
    let i2c2_config = {
        let mut config = i2c::Config::default();
        config.frequency = i2c_frequency;
    ...
    let i2c_master = I2c::new(p.I2C2, p.PB10, p.PB11, p.DMA1_CH4, p.DMA1_CH5, Irqs, i2c2_config);
            ...
            Ok(command) => match command.kind {
                ...
                SlaveCommandKind::Read => {
                    ...
                    match i2c.respond_to_read(&response_data).await {
                        Ok(status) => {
                            info!("[Slave] Sent data, status: {:?}", status);
                        }
                        Err(e) => {
...
async fn master_task(mut i2c: I2c<'static, embassy_stm32::mode::Async, i2c::mode::Master>) {
    ...
    // Expected response from slave
    const EXPECTED: [u8; 8] = [0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7];

    // Test: Combined write_read (1 byte write, 4 byte read) - WR1.1
    test_num += 1;
    info!("--- Test {}: write_read (1 write, 4 read) [WR1.1] ---", test_num);
    {
        let write_data = [0x51];
        let mut read_buf = [0u8; 4];
        match i2c.write_read(I2C_ADDR, &write_data, &mut read_buf).await {
            Ok(()) => {
                if read_buf == EXPECTED[..4] {
                    info!("[PASS] Read: {:02X}", read_buf);
                    passed += 1;
                } else {
                    error!("[FAIL] Got {:02X}, expected {:02X}", read_buf, &EXPECTED[..4]);
                    failed += 1;
                }
            }
            Err(e) => {
                error!("[FAIL] write_read error: {:?}", e);
                failed += 1;
            }
        }
    }
    ...
    // Test: Combined write_read (2 byte write, 4 byte read) - WR1.2
    test_num += 1;
    info!("--- Test {}: write_read (2 write, 4 read) [WR1.2] ---", test_num);
    {
    ...
    // Test: Combined write_read (4 byte write, 4 byte read) - WR1.3 (the failing one!)
    test_num += 1;
    info!("--- Test {}: write_read (4 write, 4 read) [WR1.3] ---", test_num);
    {
    ...
    // Test: Combined write_read (8 byte write, 8 byte read) - WR1.4
    test_num += 1;
    info!("--- Test {}: write_read (8 write, 8 read) [WR1.4] ---", test_num);
    {
        ...
        match i2c.write_read(I2C_ADDR, &write_data, &mut read_buf).await {
            Ok(()) => {
                if read_buf == EXPECTED {
                    ...
                    error!("[FAIL] Got {:02X}, expected {:02X}", read_buf, EXPECTED);
                    failed += 1;
                }
            }
            Err(e) => {
    ...
    // Test: Combined write_read (1 byte write, 8 byte read) - WR2.2 (another failing one!)
    test_num += 1;
    info!("--- Test {}: write_read (1 write, 8 read) [WR2.2] ---", test_num);
    {
        let write_data = [0x91];
        let mut read_buf = [0u8; 8];
        match i2c.write_read(I2C_ADDR, &write_data, &mut read_buf).await {
