...
async fn main(_spawner: Spawner) {
    ...
    loop {
        ...
        // RSTDAA
        i3c.async_write(0x7e, &[0x06]).await.unwrap();

        i3c.i2c_async_write_read(0x48, &[0x00], &mut buf).await.unwrap();
        ...
        // ENTDAA
        i3c.async_write(0x7e, &[0x07]).await.unwrap();

        // P3T1755 temperature register = 0x00
        i3c.async_write_read(0x48, &[0x00], &mut buf).await.unwrap();
        let raw = f32::from(i16::from_be_bytes(buf) / 16);
