...
async fn main(_spawner: Spawner) {
    ...
    let mut usart1 = Uart::new(p.USART1, p.PB6, p.PB7, p.DMA1_CH3, p.DMA1_CH4, Irqs, config1).unwrap();

    //RX1/TX1 (LPUART) on LoRa-E5 mini v1.0
    let mut usart2 = Uart::new(p.LPUART1, p.PC1, p.PC0, p.DMA1_CH5, p.DMA1_CH6, Irqs, config2).unwrap();
