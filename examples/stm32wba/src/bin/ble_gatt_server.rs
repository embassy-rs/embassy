...
use embassy_stm32::rcc::{
    mux, AHB5Prescaler, AHBPrescaler, APBPrescaler, PllDiv, PllMul, PllPreDiv, PllSource, Sysclk,
    VoltageScale,
};
...
use embassy_stm32_wpan::gatt::{
    is_cccd_handle, is_value_handle, CccdValue, CharProperties, CharacteristicHandle,
    GattEventMask, GattServer, SecurityPermissions, ServiceHandle, ServiceType, Uuid,
...
async fn main(spawner: Spawner) {
    ...
    info!(
        "Characteristic created: handle 0x{:04X}",
        data_char_handle.0
    );
    info!(
        "  Value handle: 0x{:04X}",
