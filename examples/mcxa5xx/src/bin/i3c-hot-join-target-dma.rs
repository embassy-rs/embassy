//! Two-board I3C Hot-Join loop stress test: MCXA5xx target.
//!
...
//!
//! Wiring: SCL P0_21 ↔ controller SCL, SDA P0_20 ↔ controller SDA, common GND.
        ...
        match tgt.listen().await.unwrap() {
            Event::DynamicAddressChanged => {
                ...
                match tgt.hot_join_request() {
                    ...
                    Err(HotJoinError::HasDynamicAddress) => info!("[tgt] dynamic address assigned"),
