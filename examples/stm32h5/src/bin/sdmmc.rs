...
async fn cyw43_task(
    runner: cyw43::Runner<'static, cyw43::SdioBus<SerialDataInterface<'static, 'static>, ExtiInput<'static>>>,
) -> ! {
    runner.run().await
}
...
async fn main(spawner: Spawner) {
    ...
    let wl_wake_host = ExtiInput::new(p.PD1, p.EXTI1, Pull::Down, Irqs);
    ...
    let (_net_device, mut control, runner) = cyw43::new_sdio(state, sdio, wl_wake_host, fw).await;
