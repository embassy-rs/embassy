//! STM32U5 platform glue for the JSON-driven hall lighting OxivGL demo.
...
async fn init_ltdc_layer(ltdc: &mut Ltdc<'static, peripherals::LTDC, ltdc::Rgb565>) {
    let layer_config = LtdcLayerConfig {
        ...
        window_x0: 0,
        window_x1: crate::rvt50_board::DISPLAY_WIDTH as _,
        window_y0: 0,
...
/// Run the JSON-driven hall lighting demo (LVGL + LTDC UI task).
pub async fn run_hall_demo(
    mut ltdc: Ltdc<'static, peripherals::LTDC, ltdc::Rgb565>,
    bufs: &'static mut LvglBuffers<{ super::platform::LVGL_BUF_BYTES }>,
) -> ! {
    init_ltdc_layer(&mut ltdc).await;

    let driver = LvglDriver::init(
        crate::rvt50_board::DISPLAY_WIDTH as i32,
