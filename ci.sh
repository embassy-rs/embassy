...
if ! command -v cargo-embassy-devtool &> /dev/null; then
    ...
    echo ""
    echo "    cargo install --git https://github.com/embassy-rs/cargo-embassy-devtool --bin cargo-embassy-devtool --locked"
    echo ""
    exit 1
fi
...
# Use devtool to build all combinations defined in the Cargo.tomls
