//! P-256 and P-384 drivers over `p256`, `p384` and `ecdsa`.
        ...
        mod $curve {
            ...
            mod arith {
                ...
                impl embassy_crypto::driver::$arith_trait for Driver {
                    ...
                    fn point_is_valid(p: &$dpoint) -> bool {
                        point(p).is_some()
                    }

                    fn point_add(p: &$dpoint, q: &$dpoint) -> Option<$dpoint> {
                        let sum = ProjectivePoint::from(point(p).unwrap()) + point(q).unwrap();
                        point_bytes(&sum)
                    }

                    fn point_mul(k: &$dscalar, p: &$dpoint) -> Option<$dpoint> {
                        point_bytes(&(ProjectivePoint::from(point(p).unwrap()) * scalar(k).unwrap()))
                    }

                    fn point_mul_base(k: &$dscalar) -> Option<$dpoint> {
                        point_bytes(&ProjectivePoint::mul_by_generator(&scalar(k).unwrap()))
                    }

                    fn point_lincomb(k1: &$dscalar, p1: &$dpoint, k2: &$dscalar, p2: &$dpoint) -> Option<$dpoint> {
                        let sum = <ProjectivePoint as LinearCombination<[(ProjectivePoint, Scalar); 2]>>::lincomb(&[
                            (point(p1).unwrap().into(), scalar(k1).unwrap()),
                            (point(p2).unwrap().into(), scalar(k2).unwrap()),
                        ]);
                        point_bytes(&sum)
                    }
                }
            ...
            mod ecdsa_driver {
                ...
                use elliptic_curve::group::Group;
                use embassy_crypto::Error;
                use embassy_crypto::driver::{Rng, $dpoint, $dscalar, $dsig};
                ...
                impl embassy_crypto::driver::$ecdsa_trait for Driver {
                    ...
                    fn sign(k: &$dscalar, digest: &[u8; $n], rng: &mut dyn Rng) -> Result<$dsig, Error> {
                        let d = nonzero_scalar(k)?;
                        let mut nonce_bytes = $dscalar([0u8; $n]);
                        let nonce = loop {
                            rng.fill_bytes(&mut nonce_bytes.0)?;
